...
func _integrate_forces(s):
	...
	if (shoot and not shooting):
		...
		get_node("sprite2/smoke").set_emitting(true)
	...
	if (jumping):
		...
		# This allows for example the player to shoot quickly left then right.
			...
			#get_node("sprite2").set_scale(Vector2(-1, 1))
	...
	if (on_floor):
		...
		if (lv.y < 0):
			...
			if (shoot_time < MAX_SHOOT_POSE_TIME):
				new_anim = "standing_weapon_ready"
			else:
				new_anim = "falling"
	...
	if (new_siding_left != siding_left):
		if (new_siding_left):
			get_node("sprite2").set_flip_h(true) 
		else:
			get_node("sprite2").set_flip_h(false)
		siding_left = new_siding_left
	
	# Change animation
	if (new_anim != anim and dead!=true ):
		anim = new_anim
		get_node("sprite2").play(anim)
...
func _on_player2_body_enter( body ):
	if body.has_method("is_enemy") and body.destroyed==false :
		die() 
